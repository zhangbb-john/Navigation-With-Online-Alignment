...
class MerweScaledSigmaPoints {
 public:
  ...
  Eigen::Matrix<double, N, N * 2 + 1> generateSigmaPoints(
    ...
    // logfile<<"generate sigma points"<<std::endl;
    sigmaPoints.col(0) = x;
    for (int i = 0; i < N; i++) {
      if (!subtract_) {
...
unscented_transform(const Eigen::Matrix<double, N, NSigma>& sigmaPoints,
  ...
  if (!meanFn) {
    // logfile<<"unscented_transform meanFn is not defined"<<std::endl;
    x = sigmaPoints * weightsMean;
  } else {
    // logfile<<"unscented_transform meanFn is defined"<<std::endl;
    x = meanFn(sigmaPoints, weightsMean);
  }
  if ( weightsMean(0) == 0)
    std::cout<<weightsMean<<std::endl;
  ...
  // logfile<<"P :"<<P<<std::endl<<" noise: "<<std::endl<<noise<<std::endl;
  P = P + noise;
